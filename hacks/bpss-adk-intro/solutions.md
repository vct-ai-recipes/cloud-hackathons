...
Welcome to the coach's guide for Introduction to Agents with ADK gHack (BPSS Edition). Here you will find links to specific guidance for coaches for each of the challenges.
...
This hack requires students to have access to a Google Cloud project where they can create and consume Google Cloud resources. The project should use the `gcp_medium_extra` variant to support the Cloud Run services and Compute Engine instances needed.
...
- Cloud Build (service deployments)
